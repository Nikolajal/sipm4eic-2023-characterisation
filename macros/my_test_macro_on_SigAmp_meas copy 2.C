...
void my_test_macro_on_SigAmp_meas(std::string data_dir = "./Data/", std::string outfile = "out.root")
{
  //  Load database
  database::basedir = data_dir;
  database::read_database(database::basedir + "/database.txt");

  auto gtest = get_dcr_threshold_scan("2","B2","TIFPA-IRR1");
  auto dgtest = derivate(gtest);
  ...
  for (auto [state_tag, state_vector] : states_to_loop_on)
  {
    for (auto [sensor, sensor_tag] : database::sensor_to_code)
    {
      ...
      std::string histo_title = Form("Signal amplitude of %s for state %s;signal amplitude (a.u.);entries", sensor.c_str(), state_vector.first.c_str());
      _TH1F[histo_tag] = new TH1F(histo_tag.c_str(), histo_title.c_str(), 60, 0., 60);
    }
  }
  //  Fill all histograms
  gROOT->SetBatch(true);
    ...
    for (auto current_board : state_vector.second)
    {
      for (auto [current_sensor, sensor_tag] : database::sensor_to_code)
      {
        for (auto current_channel : database::sensor_to_channels[current_sensor])
        {
          ...
          if (!current_curve)
            continue;
          _TH1F[histo_tag]->Fill( database::measure_signal_amp(current_curve, Form("./plots/hCT_%s_%s_%s", current_board.c_str(), current_channel.c_str(), current_state.c_str())).first);
        }
      }
    }
  }
  ...
  for (auto [state_tag, state_vector] : states_to_loop_on)
  {
    std::string histo_title = Form("Signal amplitude for state %s;;signal amplitude (a.u.)", state_vector.first.c_str());
  ...
  gPad->DrawFrame(15, 0, 145, 40, ";proton energy (MeV);signal amplitude (a.u.)");
  for (auto iSns = 1; iSns <= 3; iSns++)
  {
  ...
  //  --- TLegend
  TLegend *legend_pirr_nirr = new TLegend(0.12, 0.12, 0.5, 0.35);
  ...
  _TH1F["NEW_avg"]->SetMaximum(50);
  _TH1F["NEW_avg"]->SetMinimum(0);
